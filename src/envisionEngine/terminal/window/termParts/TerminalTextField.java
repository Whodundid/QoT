...
public class TerminalTextField extends WindowTextField {
	...
	public void keyPressed(char typedChar, int keyCode) {
		...
		if (keyCode == 265) { //up
			if (term.historyLine < TerminalHandler.cmdHistory.size()) {
				if (term.lastUsed == 0) {
					term.historyLine += 1;
			...
			if (term.historyLine <= TerminalHandler.cmdHistory.size() && term.historyLine > 1) {
				if (term.historyLine == TerminalHandler.cmdHistory.size() || term.lastUsed == 1) { term.historyLine -= 2; }
	...
	private String getHistoryLine(int lineNum) {
		if (!TerminalHandler.cmdHistory.isEmpty()) {
			if (lineNum == 0) { return TerminalHandler.cmdHistory.get(TerminalHandler.cmdHistory.size() - 1); }
			else if (lineNum > 0) { return TerminalHandler.cmdHistory.get(TerminalHandler.cmdHistory.size() - (lineNum + 1)); }
		}
		return null;
	}
}
