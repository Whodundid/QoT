...
/**
 * An item that can be placed and used in the map editor hotbar. Can be
 * used to reference any of the game's assets without needing to
 * individually specify a different type for each.
 */
public class EditorObject implements IDrawable {
    ...
    public EditorObject(GameObject objectIn) {
        this(objectIn, EditorObjectType.GAME_OBJECT);
    }
    public EditorObject(WorldTile tileIn) {
        this(tileIn, EditorObjectType.TILE);
    }
    public EditorObject(Entity entityIn) {
        this(entityIn, EditorObjectType.TILE);
    }
    ...
    public String getName() { return (theObject != null) ? theObject.getName() : "null"; }
    public Sprite getSprite() { return (theObject != null) ? theObject.getSprite() : null; }
    
    public boolean isSelected() { return isSelected; }
    public void setSelected(boolean val) { isSelected = val; }
    
    public static EditorObject of(WorldTile tileIn) {
        return new EditorObject(tileIn);
    }
    
    public static EditorObject of(GameObject entityIn) {
        return new EditorObject(entityIn);
    }
