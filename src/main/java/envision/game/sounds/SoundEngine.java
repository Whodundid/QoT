...
public class SoundEngine {
    ...
    public static EList<Audio> getAllPlaying() {
        return EList.unmodifiableList(currentlyPlaying);
    }
    ...
    public static Audio play(Audio in) {
        if (in != null) {
            if (EngineSettings.musicVolume.get() > 0) in.start();
    ...
    public static Audio loop(Audio in) {
        if (in != null) {
            if (EngineSettings.musicVolume.get() > 0) in.loop();
            currentlyPlaying.add(in);
        }
        return in;
    }
    ...
    public static Audio getTrackByName(String name) {
        return allSongs.getFirst(s -> s.getName().equals(name));
    }
    ...
    public static void setMusicVolume(int amount) {
        ...
        int old = EngineSettings.musicVolume.get();
        EngineSettings.musicVolume.set(val);
        //only update config if the volume actually changed
        if (EngineSettings.musicVolume.get() != old) Envision.saveEngineConfig();
    }
    
    public static int getMusicVolume() {
        return EngineSettings.musicVolume.get();
    }
