...
public class AnimationHandler {
	...
	public boolean playIfNotAlreadyPlaying(String setName) {
		if (currentAnimation == null || !currentAnimation.getSetName().equals(setName)) loadAnimation(setName);
		play();
		return true;
	}
	...
	public boolean unloadCurrentIfNotPlayingSet(String setName) {
	    if (!isPlaying(setName)) return false;
	...
	/**
	 * Checks if the set current playing matches the given set name.
	 * 
	 * @param set The set name
	 * @return True if the given set is currently playing
	 */
	public boolean isPlaying(String set) {
	    return currentAnimation != null && currentAnimation.getSetName().equals(set);
	}
	...
	public void setGlobalUpdateInterval(long interval) {
	    for (var set : animationSets.values()) {
	        set.setUpdateInterval(interval);
	    }
	}
	...
	public void setEntityBaseTexture(String setName, int index) {
	    var set = getSetInternal(setName);
	    if (set == null) return;
	    if (set.isNotEmpty() && index >= 0 && index < set.length()) {
	        origTex = set.getFrameAtIndex(index);
	    }
	}
