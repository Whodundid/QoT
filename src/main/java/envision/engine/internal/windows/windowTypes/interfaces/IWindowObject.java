package envision.engine.internal.windows.windowTypes.interfaces;

import java.util.Collection;
import java.util.function.Consumer;

import envision.Envision;
import envision.engine.internal.assets.CursorTextures;
import envision.engine.internal.inputHandlers.CursorHelper;
...
import envision.engine.internal.windows.windowObjects.utility.FocusLockBorder;
...
import envision.engine.internal.windows.windowTypes.WindowObjectProperties;
import envision.engine.internal.windows.windowTypes.WindowParent;
import envision.engine.internal.windows.windowUtil.FutureTaskEventType;
import envision.engine.internal.windows.windowUtil.FutureTaskManager;
import envision.engine.internal.windows.windowUtil.WindowObjectGroup;
import envision.engine.internal.windows.windowUtil.input.KeyboardInputAcceptor;
import envision.engine.internal.windows.windowUtil.input.MouseInputAcceptor;
import envision.engine.internal.windows.windowUtil.layouts.IWindowLayout;
import envision.engine.internal.windows.windowUtil.layouts.LayoutConstraint;
import envision.engine.internal.windows.windowUtil.windowEvents.ObjectEventHandler;
...
import envision.engine.internal.windows.windowUtil.windowEvents.events.EventActionPerformed;
...
import envision.engine.internal.windows.windowUtil.windowEvents.events.EventModify;
import envision.engine.internal.windows.windowUtil.windowEvents.events.EventMouse;
import envision.engine.internal.windows.windowUtil.windowEvents.events.EventObjectAdded;
import envision.engine.internal.windows.windowUtil.windowEvents.events.EventObjectClosed;
import envision.engine.internal.windows.windowUtil.windowEvents.events.EventObjectRemoved;
import envision.engine.internal.windows.windowUtil.windowEvents.events.EventRedraw;
import eutil.EUtil;
...
public interface IWindowObject extends KeyboardInputAcceptor, MouseInputAcceptor {
    ...
    public default void reInitChildren() {
        ...
        if (p != null && p.getModifyType() != ObjectModifyType.RESIZE) {
    ...
    public default void setLayout(IWindowLayout layoutManager) {
        properties().layoutManager = layoutManager;
        if (layoutManager != null) layoutManager.applyLayout(this, getCombinedChildren());
    }
    ...
    /**
     * Adds a task to be executed once the specified future task event type
     * event is run.
     * 
    ...
    /**
     * Returns all of the tasks that are scheduled to be run once the given
     * future task event type is fired.
     * 
    ...
    /**
     * Internal event fired once this object has been fully initialized
     * with all of its children.
     * <p>
     * Most developers will probably want to override the non-internal
     * 'onInit' method instead of this one.
     * <p>
    ...
    /**
     * Internal event fired from the top parent when the object is drawn
     * for the first time.
     * <p>
     * Most developers will probably want to override the non-internal
    ...
    /**
     * Internal event fired when this object recieves focus.
     * <p>
     * Most developers will probably want to override the non-internal
     * 'onInitialFocusGained' method instead of this one.
     * <p>
     ...
     * and internal object properties will not be set unless manually set.
     */
    public default void onFocusGained_i(EventFocus event) {
        onFocusGained(event);
        var ftm = getFutureTaskManager();
        if (ftm != null) ftm.runTaskType(FutureTaskEventType.ON_FOCUS_GAINED);
    }
    
    /**
     * Internal event fired when this object loses focus.
     * <p>
     * Most developers will probably want to override the non-internal
     * 'onInitialFocusGained' method instead of this one.
     * <p>
     ...
     * and internal object properties will not be set unless manually set.
     */
    public default void onFocusLost_i(EventFocus event) {
        onFocusLost(event);
        var ftm = getFutureTaskManager();
        if (ftm != null) ftm.runTaskType(FutureTaskEventType.ON_FOCUS_LOST);
    }
    
    /**
     * Internal event called when this object has actually been added to
     * its parent.
     * <p>
     * Most developers will probably want to override the non-internal
    ...
    /**
     * Event fired from this object's pre draw setup to perform cursorImage
     * changes.
     */
    ...
    public default void updateBeforeNextDraw(int mXIn, int mYIn) {
        ...
        if (!properties().childrenToBeRemoved.isEmpty()) {
            for (var o : properties().childrenToBeRemoved) {
                ...
                postEvent(new EventObjectRemoved(this, o));
            }
            properties().childrenToBeRemoved.clear();
            if (properties().layoutManager != null) properties().layoutManager.applyLayout(this, getChildren());
        }
        ...
        if (!properties().childrenToBeAdded.isEmpty()) {
            for (var o : properties().childrenToBeAdded) {
                ...
                postEvent(new EventObjectAdded(this, o));
            }
            properties().childrenToBeAdded.clear();
            if (properties().layoutManager != null) {
                properties().layoutManager.applyLayout(this, getChildren());
            }
        }
    }
    ...
    /**
     * Returns the dimensions of this object without dimension boundary limitations.
     * <p>
     ...
     */
    public default boolean isParentMoving() { return isObjectMoving(getParent()); }
     ...
     * coordinates. The specified position represents the top left corner
     * of this object. All children will remain in their original positions
    ...
    public default WindowObjectGroup getObjectGroup() { return properties().objectGroup; }
    /** Sets this object's object group. */
    public default void setObjectGroup(WindowObjectGroup groupIn) { properties().objectGroup = groupIn; }
    /** Creates a new group for this object with this object as the parent and all member's as children. */
    public default WindowObjectGroup createObjectGroup(IWindowObject... members) {
        WindowObjectGroup group = new WindowObjectGroup(this, members);
        setObjectGroup(group);
        for (IWindowObject o : members) o.setObjectGroup(group);
        return group;
    }
    /** Event fired when any object within the object group fires an event. */
    public default void onGroupNotification(WindowObjectEvent e) {}
    ...
    public default EList<IWindowObject> getVisibleChildren() { return getChildren().filter(c -> c.willBeDrawn()); }
    ...
    public default boolean containsObject(IWindowObject object) { return getCombinedChildren().contains(object); }
    /** Returns true if the specified object type is a child of the parent or is being added to the parent. */
    public default boolean containsObject(Class<?> objIn) { return (objIn != null) && getAllChildren().anyMatch(objIn::isInstance); }
    ...
    public default void addObject(IWindowObject object, LayoutConstraint layoutConstraint) {
        try {
            ...
            if (getCombinedChildren().contains(object)) {
                System.err.println(this + " already contains " + object + "!");
                return;
            }
    ...
    public default void removeObject(IWindowObject... objs) {
        ...
        getRemovingChildren().addA(objs);
    }
    
    /**
     * Removes all children from this object including all children that were
     * about to be added.
     */
    public default void removeAll() {
        // remove all children about to be added
    ...
    /** Returns the object that will receive focus by default when the base object has focus transfered to it. */
    public default IWindowObject getDefaultFocusObject() { return properties().defaultFocusObject; }
    /** Sets a default focus object for this object. When the main object receives focus, the top
     *  parent will attempt to transfer focus to the specified default focus object. */
    public default void setDefaultFocusObject(IWindowObject objectIn) { properties().defaultFocusObject = objectIn; }
    ...
    /**
     * Signals the top parent to transfer focus from this object to the top
     * parent's default focus object on the next draw cycle. If this object
     * has a focus lock set, the lock will be removed and focus will be
     * transfered to the top parent's default focus object on the next draw
     * cycle.
     */
    ...
    /** Signals the top parent to try transferring focus to this object on the next draw cycle.
     *  If another object has a focus lock, this object will not receive focus */
    public default void requestFocus() { requestFocus(FocusType.TRANSFER); }
    /** Same as the previous request focus but the exact type of focus event can be specified. */
    public default void requestFocus(FocusType typeIn) { getTopParent().setObjectRequestingFocus(this, typeIn); }
    ...
    public default void mouseEntered(int mX, int mY) { postEvent(new EventMouse(this, mX, mY, -1, MouseType.ENTERED)); }
    /** Event fired upon the mouse exiting this object. */
    public default void mouseExited(int mX, int mY) { postEvent(new EventMouse(this, mX, mY, -1, MouseType.EXITED)); }
    
    /**
     * Returns true if the mouse is currently inside this object regardless
     * of z-level. If a boundary enforcer is set, this method will return
     * true if the mouse is inside of the the specified boundary.
     */
    ...
    public default void registerListener(IWindowObject objIn, WindowEventType... eventTypes) {
        var handler = properties().eventHandler;
        if (handler != null) handler.registerObject(objIn, eventTypes);
    }
    
    /** Unregister a listener Object. */
    public default void unregisterListener(IWindowObject objIn, WindowEventType... eventTypes) {
        var handler = properties().eventHandler;
        if (handler != null) handler.unregisterObject(objIn, eventTypes);
    }
    
    /** Broadcasts an ObjectEvent on this object. */
    public default boolean postEvent(WindowObjectEvent e) {
        var handler = properties().eventHandler;
    ...
    public default void onEvent(WindowObjectEvent e) {}
    ...
    public default void actionPerformed(IActionObject object, Object... args) {
        postEvent(new EventActionPerformed(this, object, args));
    }
    ...
    /** Removes this object and all of it's children from the immediate a parent.
     *  Removes any present focus locks on this object and returns focus back to the top parent. */
    ...
    public default void close(boolean recursive) {
        ...
        postEvent(new EventObjectClosed(this));
    ...
    public static void setVal(Consumer<? super IWindowObject> action, IWindowObject... objs) { EUtil.filterNullForEachA(action, objs); }
    ...
    public static void setVal(Consumer<? super IWindowObject> action, Collection<IWindowObject> objs) { EUtil.filterNullForEach(objs, action); }
