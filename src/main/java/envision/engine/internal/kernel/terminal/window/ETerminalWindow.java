package envision.engine.internal.kernel.terminal.window;
...
import envision.engine.internal.assets.TaskBarTextures;
...
import envision.engine.internal.kernel.user.UserProfileRegistry;
...
public class ETerminalWindow extends WindowParent implements EnvisionConsoleOutputReceiver, EnvisionLangErrorCallBack {
    ...
    protected double vScrollRate = 35.0;
    ...
    protected EList<UserProfile> userStack = EList.newList();
    
    protected String watchCommand;
    protected float watchCommandInterval;
    protected float timeSincelastWatchCommand;
    protected EList<String> watchCommandArguments;
    
    protected EnvisionProgram termrcProgram;
    protected EnvisionProgramRunner termrcRunner;
    protected TerminalCommandHandler commandHandler;    
    //==============
    // Constructors
    //==============
    
    public ETerminalWindow() {
        this(EnvisionKernel.getActiveUser());
    }
    
    public ETerminalWindow(UserProfile user) {
        super();
        
        userStack.push(user);
        
        aliases.add("terminal", "console", "term");
        dir = DeveloperDesktop.DESKTOP_DIR;
        windowIcon = TaskBarTextures.terminal;
        
        commandHandler = new TerminalCommandHandler();
    ...
    public void initWindow() {
        ...
        vScrollRate = EngineSettings.termScrollRate.getDouble();
    ...
    public void initChildren() {
        ...
        history.setVScrollRate(vScrollRate);
        ...
        var objectGroup = new WindowObjectGroup(this);
        ...
        if (!init) {
            ...
            EColors uc = UserProfileRegistry.getProfileEColor(getActiveUser());
            writeln(uc, getActiveUser().getName(), EColors.white, ":");
            writeln();
            init = true;
        }
    ...
    private void loadTermrc() {
        File termrc = DeveloperDesktop.DESKTOP_TERMINAL_LAUNCH_SCRIPT;
        if (!EFileUtil.fileExists(termrc)) return;
        
        try {
            List<String> lines = Files.readAllLines(termrc.toPath());
            termrcProgram = new EnvisionProgram("termrc", lines);
            
            termrcProgram.setEnableBlockStatements(false);
            termrcProgram.setEnableBlockStatementParsing(false);
            termrcProgram.setConsoleReceiver(this);
            termrcProgram.setErrorCallback(this);
            
            termrcProgram.addJavaObjectToProgram("term", this);
            
            termrcRunner = new EnvisionProgramRunner(termrcProgram);
            termrcRunner.start();
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    public void drawObject(float dt, int mXIn, int mYIn) {
        if (header != null) {
            ...
            if (UserProfile.isUserDev(getActiveUser())) {
    ...
    public void onTickUpdate_i(float dt) {
        ...
        if (watchCommand != null) {
            timeSincelastWatchCommand += dt;
            if (timeSincelastWatchCommand >= watchCommandInterval) {
                clear();
                runCommand(watchCommand, watchCommandArguments, false);
                timeSincelastWatchCommand = 0f;
            }
        }
    }
        ...
        try {
            if (isTab) {
                ...
                if (cmd.equals("y") || cmd.equals("n")) {
                    ...
                    confirmationCommand.preRun(this, previousArgs, prevRunVisually);
                ...
                commandHandler.cmdHistory.add(cmd);
                inputField.setText("");
                scrollToBottom();
            }
            else {
                ...
                if (!isClear) {
                    EColors uc = UserProfileRegistry.getProfileEColor(getActiveUser());
                    writeln(uc, getActiveUser().getName(), EColors.white, ": ", cmd);
                }
     ...
     */
    public void runCommand(String input) { runCommand(input, true); }
    public void runCommand(String input, boolean addToCmdHistory) {
        if (addToCmdHistory) commandHandler.cmdHistory.add(input);
        commandHandler.executeCommand(this, input, false);
        scrollToBottom();
    }
    ...
    public void runCommand(String command, EList<String> arguments) {
        runCommand(command, arguments, true);
    }
     ...
     * @param input The command to run
     */
    public void runCommand(String command, EList<String> arguments, boolean addToCmdHistory) {
        String argString = EStringUtil.toString(arguments);
        if (addToCmdHistory) commandHandler.cmdHistory.add(command + ((!argString.isBlank()) ? " " + argString : ""));
        commandHandler.executeOnTerminal(this, command, arguments);
        scrollToBottom();
    }
    ...
    public ETerminalWindow scrollToBottom() {
        ...
        synchronized (history) {
            if (!history.isInitialized()) return this;
        ...
        try {
            if (!input.isEmpty()) {
                ...
                if (!isCommand && getCurrentArg() >= 1) {
                    commandHandler.executeCommand(this, input, true);
                }
                //build completions off of partial command input
                else if (startArgPos == -1 || getCurrentArg() == 0 && !tab1 && !isCommand) {
                    isCommand = true;
                    try {
                        EList<String> options = commandHandler.buildTabCompleteCommandOptions(input);
                ...
                if (!tab1) {
                    EList<String> cnames = commandHandler.getSortedCommandNames(getActiveUser());
    ...
    public ETerminalWindow buildTabCompletions(Collection<String> dataIn) {
        ...
        for (int i = 1; i < tabData.size() + 1; i++) {
        ...
        int amount = tabData.size();
        int cur = 1;
        var line = new EStringBuilder();
        
        var it = tabData.iterator();
    ...
    public ETerminalWindow clear() {
        if (history != null) history.clear();
        EColors uc = UserProfileRegistry.getProfileEColor(getActiveUser());
        writeln(uc, getActiveUser().getName(), EColors.white, ":");
        if (EngineSettings.termCmdNewLines.getBoolean()) writeln();
        return this;
    }
    
    public ETerminalWindow clearTabData() {
        tabData.clear();
        return this;
    }
    ...
    private void parseText(String msgIn, int colorIn) {
        ...
        synchronized (history) {
            for (String s : lines) {
                ...
                if (history.isInitialized()) history.addTextLine(line);
            }
        }
    ...
    public UserProfile getActiveUser() { return userStack.peek(); }
    public TerminalCommandHandler getCommandHandler() { return commandHandler; }
    ...
    public void switchUser(UserProfile profile) {
        userStack.push(profile);
    }
    
    public void popUser() {
        if (userStack.size() == 1) return;
        userStack.pop();
    }
    ...
    public void setTerminalScrollRate(double rate) {
        vScrollRate = rate;
        if (history != null) history.setVScrollRate(vScrollRate);
    }
    ...
    public synchronized void setWatchCommand(String cmd, EList<String> args, float interval) {
        watchCommand = cmd;
        watchCommandArguments = args;
        watchCommandInterval = interval;
        timeSincelastWatchCommand = 0f;
    ...
    public synchronized void killCurrentCommand() {
        watchCommand = null;
        watchCommandArguments = null;
        watchCommandInterval = 0f;
        timeSincelastWatchCommand = 0f;
        writeln("terminated");
    }
