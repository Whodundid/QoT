...
import envision.engine.internal.kernel.terminal.commands.ListableCommand;
...
public class CMD_Help extends TerminalCommand implements ListableCommand {
    ...
    public void handleTabComplete(ETerminalWindow termIn, EList<String> args) {
        EList<String> options = termIn.getCommandHandler().getSortedCommandNames(user());
    ...
    public void runCommand() {
        ...
        var cmds = termHandler().getCommandNames();
        if (!cmds.contains(commandName)) {
            error("Unrecognized command name");
            return;
        }
        
        TerminalCommand command = termHandler().getCommand(commandName);
    ...
    public void showHelp(ETerminalWindow termIn) {
        ...
        for (var box : termHandler().getSortedCommands(user())) {
            //print category name
    ...
    public EList<String> getList() {
        if (term() == null) return EList.newList();
        return termHandler().getSortedCommandNames(user());
    }
