...
public class WindowScrollBar extends ActionObject {
    ...
    /**
     * Performs the equivalent of 'getScrollPos() - getVisibleAmount()'.
     * 
     * @return The actual scroll position starting at zero
     */
    public double calcScrollOffset() {
        return scrollPos - visibleAmount;
    }
    ...
    private void moveThumb(double newX, double newY) {
        if (vertical) {
            thumbStartY += newY;
            thumbEndY += newY;
            thumbStartY = ENumUtil.clamp(thumbStartY, startY, endY - thumbSize);
            thumbEndY = ENumUtil.clamp(thumbEndY, startY + thumbSize, endY);
        }
        else {
            thumbStartX += newX;
            thumbEndX += newX;
            thumbStartX = ENumUtil.clamp(thumbStartX, startX, endX - thumbSize);
            thumbEndX = ENumUtil.clamp(thumbEndX, startX + thumbSize, endX);
        }
        calculateScrollPos();
    ...
    private void calculateScrollPos() {
        ...
        if (vertical) {
            equation = (int) (thumbStartY - startY + 0.5) / (height - thumbSize);
        }
        else {
            equation = (int) (thumbStartX - startX + 0.5) / (width - thumbSize);
        }
        
        double relativeThumbPos = ENumUtil.clamp(equation, 0f, 1f);
    ...
    public void setScrollPos(double pos) {
        ...
        if (vertical) {
            thumbStartX = startX;
            thumbStartY = startY + (scrollPos - visibleAmount) * interval;
            thumbStartY = ENumUtil.clamp(thumbStartY, startY, thumbStartY);
            thumbEndX = endX;
            thumbEndY = thumbStartY + thumbSize;
        }
        else {
            thumbStartX = startX + (scrollPos - visibleAmount) * interval;
            thumbStartX = ENumUtil.clamp(thumbStartX, startX, thumbStartX);
            thumbStartY = startY;
