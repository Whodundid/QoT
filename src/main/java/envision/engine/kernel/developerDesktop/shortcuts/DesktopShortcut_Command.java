package envision.engine.kernel.developerDesktop.shortcuts;
...
public class DesktopShortcut_Command extends DesktopShortcut {
    ...
    public void openShortcut() {
        ...
        if (willOpenTerminal) {
            ...
            var handler = DUMMY_TERMINAL.getCommandHandler();
            // find a registered command under the given name
            //TerminalCommand command = handler.getRegisteredCommand(commandName);
            //if (command == null) return;
            
            String cmd = commandName + " " + EStringUtil.combineAll(args, " ");
            handler.executeCommand(DUMMY_TERMINAL, cmd);
            //command.preRun(DUMMY_TERMINAL, args, false);
        }
    }
