package envision.engine.kernel.terminal.commands;
...
import envision.engine.kernel.process.EnvisionProcess;
import envision.engine.kernel.terminal.TerminalCommandHandler;
import envision.engine.kernel.terminal.terminalUtil.ArgHelper;
import envision.engine.kernel.terminal.terminalUtil.TermArgLengthException;
import envision.engine.kernel.terminal.terminalUtil.TermArgParsingException;
import envision.engine.kernel.terminal.terminalUtil.TerminalCommandError;
import envision.engine.kernel.terminal.window.ETerminalWindow;
import envision.engine.kernel.terminal.window.TerminalTextField;
 ...
 * @author Hunter Bragg
 */
public abstract class TerminalCommand extends EnvisionProcess {
    ...
    /** Defines the number of arguments that this command will expect. A value of '-1' indicates infinite. */
    ...
    protected void parseCommandModifiers(EList<String> args) {
        ...
        while (it.hasNext()) {
            ...
            if (s.startsWith("-") && s.length() > 1) {
                ...
                if (allowAnyModifier) {
                    parsedModifiers.add(mod);
                    it.remove();
                }
                else if (mod.length() == 1 && acceptedModifiers.contains(mod)) {
                    parsedModifiers.add(mod);
                    it.remove();
                }
                else {
                    ...
                    for (String m : parts) {
                        if (acceptedModifiers.contains(m)) {
    ...
    @Override
    public void run() {
        // don't run if the command's backend was never setup
        if (!setup) return;
        
        // actually run the command's code
        try {
    ...
    public EList<String> subArgs(int start) { return EList.wrap(argHelper.stringArgs().subList(start, argLength())); }
    public EList<String> subArgs(int start, int end) { return EList.wrap(argHelper.stringArgs().subList(start, end)); }
    public int argLength() { return argHelper.stringLength(); }
    public ETerminalWindow term() { return term; }
    public TerminalCommandHandler termHandler() { return term.getCommandHandler(); }
    ...
    public UserProfile user() { return term.getActiveUser(); }
    ...
    /** @return The result of this command after it is run (null if not run yet!). */
    ...
    protected int parseInt(int argNum) { return parseInt(argNum, ERROR_EXPECTED_INT); }
    protected int parseInt(int argNum, String onErrorString) {
        try { return Integer.parseInt(arg(argNum)); }
        catch (Exception e) { throw new TermArgParsingException(e, onErrorString); }
    }
    protected int parseInt(int argNum, int defaultValue, String onErrorString) {
        try { return Integer.parseInt(arg(argNum)); }
    ...
    protected double parseDouble(int argNum) { return parseInt(argNum, ERROR_EXPECTED_NUMBER); }
    protected double parseDouble(int argNum, String onErrorString) {
        try { return Double.parseDouble(arg(argNum)); }
        catch (Exception e) { throw new TermArgParsingException(e, onErrorString); }
    }
    protected double parseDouble(int argNum, double defaultValue, String onErrorString) {
        try { return Double.parseDouble(arg(argNum)); }
        catch (Exception e) { error(e, onErrorString); }
        return defaultValue;
    }
