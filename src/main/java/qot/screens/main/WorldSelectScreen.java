...
public class WorldSelectScreen extends GameScreen {
    ...
    private void loadDefault() {
        File f = new File(CurrentGame.getEditorWorldsDir(), "new");
        if (f.exists()) {
            loadWorld(f);
        }
    }
    ...
    private void openMapChooser() {
        explorer = new FileExplorerWindow(this, CurrentGame.getEditorWorldsDir(), true);
    ...
    private void loadLastEditor() {
        ...
        if (last.isBlank() || last.isEmpty()) error = "There is no last editor map!";
            ...
            File worldFile = new File(CurrentGame.getEditorWorldsDir(), last);
            loadWorld(worldFile);
        }
    }
