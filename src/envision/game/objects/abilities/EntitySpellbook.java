...
public class EntitySpellbook {
	...
	public void drawAbilities(GameScreen screen) {
		...
		for (int i = 0; i < size; i++) {
			...
			double sw = FontRenderer.getStringWidth("" + level) * 0.5;
			
			GLObject.drawTexture(tex, sx, sy, width, height);
...
//				System.out.println(amount + " : " + max + " : " + ratio + " : " + (sy + height - (height * ratio)));
				...
				GLObject.drawRect(sx, drawY, sx + width, sy + height, EColors.vdgray.opacity(200));
			}

			GLObject.drawString(level, sx + width * 0.5 - sw, sy + height * 0.75, EColors.green);
		}
	}
