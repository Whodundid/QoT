...
public class EArrayList<E> extends AbstractList<E> {
	...
	public EArrayList(Stream<E> streamIn) {
		int size = 0;
		EArrayList<E> tempList = null;
		
		if (streamIn != null) {
			tempList = new EArrayList();
			
			Iterator<E> it = streamIn.iterator();
			while (it.hasNext()) { tempList.add(it.next()); }
			
			size = tempList.size();
		}
		
		if (size > 0) { elementData = new Object[size]; }
		else if (size == 0) { elementData = EMPTY_ELEMENTDATA; }
		else { throw new IllegalArgumentException("Illegal Capacity: " + size); }
		
		if (size > 0 && tempList != null) { addAll(tempList); }
	}
	...
	public EArrayList<E> print(boolean count) {
		for (int i = 0; i < size(); i++) {
			System.out.println(((count) ? i + ": " : "") + elementData[i]);
		}
		return this;
	}
