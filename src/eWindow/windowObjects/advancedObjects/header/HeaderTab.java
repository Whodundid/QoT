package eWindow.windowObjects.advancedObjects.header;
...
public class HeaderTab extends WindowButton {
	...
	protected StorageBox<Double, Double> mousePos = new StorageBox();
	protected StorageBox<Double, Double> startPos = new StorageBox();
	...
	public void drawObject(int mXIn, int mYIn) {
		...
		try {
			...
			for (int i = 0; i < actual.length(); i++) {
				if (getStringWidth(drawString) <= width - 13) {
	...
	protected void checkForMove(double mX, double mY) {
		/*
		if (!windowCreated) {
	...
	protected boolean checkMouseDrag(double mX, double mY) {
		ITopParent p = getTopParent();
		if (p != null) {
			//System.out.println("is mouse in header: " + p.isMouseInsideHeader(mX, mY));
		}
		StorageBox<Double, Double> testBox = new StorageBox(mX, mY);
		double dist = NumUtil.getDistance(mousePos, testBox);
