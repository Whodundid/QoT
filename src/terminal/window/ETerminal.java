...
import terminal.window.termParts.TerminalTextField;
import terminal.window.termParts.TerminalTextLine;
...
public class ETerminal<E> extends WindowParent<E> {
	...
	public void initWindow() {
		...
		int w = NumberUtil.clamp(1150, 200, Game.getWindowSize().getWidth());
		int h = NumberUtil.clamp(600, 200, Game.getWindowSize().getHeight());
		setDimensions(w, h);
		setMinDims(70, 32);
		setResizeable(true);
		setMaximizable(true);
	...
	public void initObjects() {
		...
		inputField = new TerminalTextField(this, startX + 2, endY - 30, width - 6, 28);
	...
	public void postReInit() {
		...
		inputField.setCursorPosition(textPos);
	}
	...
	public void handleTab() {
		...
		try {
			if (!input.isEmpty()) {
				...
				if (!isCommand && getCurrentArg() >= 1) {
					...
					if (!isCommand) {
						...
						try {
							...
							for (String s : TerminalHandler.getInstance().getSortedCommandNames()) {
				...
				if (!tab1) {
					EArrayList<String> cnames = TerminalHandler.getInstance().getSortedCommandNames();
